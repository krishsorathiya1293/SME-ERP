...
public class ItemServiceImpl
    extends AbstractCrudServiceV1<ItemEntity, NewItem, Item>
  ...
  @Override
  protected JpaRepository<ItemEntity, Long> repository() {
    return itemRepository;
  }

  @Override
  protected EntityMapper<ItemEntity, NewItem, Item> mapper() {
    return itemMapper;
  }
  ...
  protected void afterCreate(ItemEntity entity, NewItem request) {
    ...
    entity.setSubCategory(subCategoryMapper.toEntity(
  ...
  public PaginatedResultItem getAll(GetAllQuery<String> query) {
    Specification<ItemEntity> spec = Specification
        .where(itemRepository.filterByStatus(query.filter()))
        .and(itemRepository.filterBySearch(query.search()));
    Page<ItemEntity> results = itemRepository.findAll(
        spec, PaginationUtils.getPageRequest(query.page(), query.size(), query.direction(), query.sortBy()));
    return PageMapper.toResult(results, itemMapper::toDomain, PaginatedResultItem::new, PaginatedResultItem::setData);
  }
}
